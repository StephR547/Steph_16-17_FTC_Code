...
public class Red_Auton extends LinearOpMode{
        ...
        public void runOpMode() throws InterruptedException {
            ...
            //Turn Right
            gTurn(.1,-2);
            ForwardLine(.1,.1,300,WHITE_THRESHOLD + 0.1);
      ...
      /* if (red2 > blue2) {
            ...
            BackLine(.2,.2,300,WHITE_THRESHOLD + 0.1);
       ...
       */
        ...
        public void FwdGLine(double powerLeft, double powerRight, int distance, double WHITE_THRESHOLD, int degrees) throws InterruptedException {
            ...
            while ((Math.abs(count) < distance) && opModeIsActive() && (Math.abs(count) < distance/2 || odsfront.getRawLightDetected() < WHITE_THRESHOLD)) {
                ...
                if (direction < 0){
                    motorLeft.setPower(-powerLeft *.5);
                    motorRight.setPower(-powerRight);
                }
                else if (direction > 0){
                    motorLeft.setPower(-powerLeft);
                    motorRight.setPower(-powerRight *.5);
                }
                else {
                    motorLeft.setPower(-powerLeft);
                    motorRight.setPower(-powerRight);
